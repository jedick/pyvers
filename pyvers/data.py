...
class FileDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        def get_data(fold, directory):
            ...
            all_data = {
                k: [item for sublist in all_data[k] for item in sublist]
                for k in all_data
            }
            return all_data
        ...
        if stage == "fit":
            ...
            self.train_dataset = PyversDataset(
                train_data, self.model_name, self.max_length
            )
        ...
        if stage == "test":
            ...
            self.test_dataset = PyversDataset(
                test_data, self.model_name, self.max_length
            )
...
class ToyDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        self.train_dataset = PyversDataset(
            train_data, self.model_name, max_length=self.max_length
        )
        ...
        self.test_dataset = PyversDataset(
            test_data, self.model_name, max_length=self.max_length
        )
...
class NLIDataModule(pl.LightningDataModule):
    ...
    def __init__(
        self, dataset_name, model_name, batch_size=32, max_length=128, test_fold="test"
    ):
        super().__init__()
